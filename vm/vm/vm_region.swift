...
public func lookAllVMRegions() {
    ...
    let vm_info = UnsafeMutablePointer<Int32>.allocate(capacity: MemoryLayout<vm_region_basic_info>.size/4)
    
    defer {
        vm_address.deinitialize(count: 1)
        vm_address.deallocate()
        
        vm_info.deinitialize(count: MemoryLayout<vm_region_basic_info>.size/4)
        vm_info.deallocate()
    }
    var regionCount = 0
    ...
    while vm_address.pointee <= MACH_VM_MAX_ADDRESS {
        ...
        var msg_count = mach_msg_type_number_t(VM_REGION_BASIC_INFO)
        var mach_port: mach_port_t = 0
        
        let ret = mach_vm_region(mach_task_self_, vm_address, &vm_size, VM_REGION_BASIC_INFO, vm_info, &msg_count, &mach_port)
        if ret != KERN_SUCCESS {
            break
        }
        ...
        let info = UnsafeMutablePointer<vm_region_basic_info>(OpaquePointer(vm_info))
